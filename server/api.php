...
function get_user_expiry_from_router(array $config, string $username): string
{
    ...
    if (isset($result[0]) && isset($result[0]['comment'])) {
        ...
        // Recherche d'une date dans le commentaire (format YYYY-MM-DD HH:MM:SS)
        if (preg_match('/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $comment, $matches)) {
            return $matches[1];
        }
        // Si le commentaire contient "expires-in", on calcule
    ...
    // Fallback : on retourne une date générique
...
switch ($route) {
    ...
    case 'expiry':
        ...
        try {
            // On essaie d'abord d'interroger le routeur
            ...
            // Dernier recours : date générique
