...
if (empty($_SESSION['finances_csrf'])) $_SESSION['finances_csrf'] = bin2hex(random_bytes(32));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (!hash_equals($csrf, (string)($_POST['csrf_token'] ?? ''))) throw new RuntimeException('Requête refusée : token CSRF invalide.');
...
try {
    ...
    $inferredCount = (int)$sales['inferred_count'];
...
<div class="container-fluid mt-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3"><div><div class="text-uppercase small fw-semibold text-muted">Business Intelligence</div><h2 class="mb-1">Finances</h2><p class="text-muted mb-0">Dépenses et résultat calculés avec la même source transactionnelle que Business et Rapports.</p></div><form method="get"><select class="form-select form-select-sm" name="periode" onchange="this.form.submit()"><option value="mois_courant" <?= $periode==='mois_courant'?'selected':'' ?>>Mois en cours</option><option value="mois_dernier" <?= $periode==='mois_dernier'?'selected':'' ?>>Mois dernier</option><option value="global" <?= $periode==='global'?'selected':'' ?>>Vue globale</option></select></form></div>
    ...
    <div class="alert alert-info py-2"><i class="bi bi-database-check"></i> Source recettes : <strong><?= htmlspecialchars($sourceLabel,ENT_QUOTES,'UTF-8') ?></strong>. Les anciennes lignes `sales_log` ne sont pas comptées comme ventes.</div>
    <div class="card card-custom"><div class="card-header card-header-custom"><i class="bi bi-plus-circle"></i> Enregistrer une dépense</div><div class="card-body"><form method="post" class="row g-2 align-items-end"><input type="hidden" name="action" value="add"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf,ENT_QUOTES,'UTF-8') ?>"><input type="hidden" name="periode" value="<?= htmlspecialchars($periode,ENT_QUOTES,'UTF-8') ?>"><div class="col-md-4"><label class="form-label">Description</label><input type="text" class="form-control" name="description" maxlength="500" required></div><div class="col-md-2"><label class="form-label">Catégorie</label><select class="form-select" name="category" required><option value="" disabled selected>Choisir…</option><?php foreach(FINANCE_CATEGORIES as $category): ?><option value="<?= htmlspecialchars($category,ENT_QUOTES,'UTF-8') ?>"><?= htmlspecialchars($category,ENT_QUOTES,'UTF-8') ?></option><?php endforeach; ?></select></div><div class="col-md-2"><label class="form-label">Montant (FCFA)</label><input type="number" class="form-control" name="amount" min="1" step="1" required></div><div class="col-md-2"><label class="form-label">Date</label><input type="date" class="form-control" name="expense_date" value="<?= date('Y-m-d') ?>" required></div><div class="col-md-2"><button type="submit" class="btn btn-orange w-100"><i class="bi bi-save"></i> Enregistrer</button></div></form></div></div>
    <div class="row g-3 mt-1"><div class="col-12 col-md-4"><div class="card card-custom p-3 h-100"><div class="stat-value text-success"><?= number_format($totalRecettes,0,',',' ') ?> FCFA</div><div class="stat-label">CA · <?= htmlspecialchars($periodLabel) ?></div></div></div><div class="col-12 col-md-4"><div class="card card-custom p-3 h-100"><div class="stat-value text-danger"><?= number_format($totalDepenses,0,',',' ') ?> FCFA</div><div class="stat-label">Dépenses · <?= htmlspecialchars($periodLabel) ?></div></div></div><div class="col-12 col-md-4"><div class="card card-custom p-3 h-100"><div class="stat-value <?= $beneficeReel >= 0 ? 'text-success':'text-danger' ?>"><?= number_format($beneficeReel,0,',',' ') ?> FCFA</div><div class="stat-label">Résultat · Marge <?= $margin===null?'—':number_format($margin,1,',',' ').' %' ?></div></div></div></div>
    <div class="card card-custom mt-3"><div class="card-header card-header-custom"><i class="bi bi-shield-check"></i> Qualité du ledger</div><div class="card-body"><div class="row g-3"><div class="col-6 col-md-3"><strong><?= number_format($rawRows,0,',',' ') ?></strong><div class="text-muted small">Transactions dans le ledger</div></div><div class="col-6 col-md-3"><strong><?= number_format(max(0,$rawRows-$duplicatesRemoved),0,',',' ') ?></strong><div class="text-muted small">Transactions retenues</div></div><div class="col-6 col-md-3"><strong><?= number_format($inferredCount,0,',',' ') ?></strong><div class="text-muted small">Historiques inférés</div></div><div class="col-6 col-md-3"><strong><?= number_format($duplicatesRemoved,0,',',' ') ?></strong><div class="text-muted small">Doublons techniques</div></div></div></div></div>
