...
// passent par api.php?route=hotspot-user(s)... — voir RAPPORT_H2.md.

require_once __DIR__ . '/../db.php';
$config = require __DIR__ . '/../config.php';

// Le token admin est injecté côté serveur, comme pour admin/status.php,
// afin que le JavaScript puisse appeler l'API sans exposer de session.
$adminToken = $config['admin']['token'] ?? (getenv('ADMIN_TOKEN') ?: '');

// Liste des profils pour les filtres/formulaires : lecture directe de
// hotspot_profiles (Supabase) uniquement. On n'utilise volontairement
// pas de repli routeur ici (new Hotspot(...)) : le H2 impose une page
// API-first, et un éventuel repli routeur échouerait de toute façon
// depuis l'hébergement Render (Render → 192.168.88.1 impossible, §19).
$profiles = [];
try {
...
<style>
    ...
    @media (max-width: 767px) {
        #usersTableWrap { display: none; }
    }
    @media (min-width: 768px) {
        #usersCardsWrap { display: none; }
    }
...
<div class="card card-custom">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-warning sync-banner" id="apiErrorBanner" role="alert"></div>

        <!-- KPI -->
        <div class="row g-2 mb-3">
            <div class="col-6 col-md-3">
                <div class="u-kpi"><div class="val" id="kpiTotal">–</div><div class="lbl">Total</div></div>
            </div>
            <div class="col-6 col-md-3">
                <div class="u-kpi"><div class="val" id="kpiActive">–</div><div class="lbl">Actifs</div></div>
            </div>
            <div class="col-6 col-md-3">
                <div class="u-kpi"><div class="val" id="kpiDisabled">–</div><div class="lbl">Désactivés</div></div>
            </div>
            <div class="col-6 col-md-3">
                <div class="u-kpi"><div class="val" id="kpiExpiring">–</div><div class="lbl">Expirants (7j)</div></div>
            </div>
        </div>
        ...
        <div class="modal-content">
            <form id="userForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-2" id="fmPasswordWrap">
...
(function () {
    ...
    async function hotspotApi(route, { method = 'GET', params = {}, body = null } = {}) {
        ...
        if (!res.ok || json.success === false) {
            const err = (json && json.error) ? json.error : { code: 'UNKNOWN', message: 'Erreur inconnue.' };
    ...
    function fmtExpiry(v) {
        if (!v) return '-';
        return v; // déjà au format AAAA-MM-JJ HH:MM:SS
    }
    ...
    function renderPagination(p) {
        ...
        for (let i = start; i <= end; i++) {
            list.appendChild(item(String(i), i, false, i === p.page));
        }
    ...
    document.getElementById('fSearch').addEventListener('input', (e) => {
        ...
        state.searchDebounce = setTimeout(() => {
            ...
            state.page = 1;
            loadUsers();
        }, 400); // debounce : pas de requête à chaque caractère
    });
    ...
    async function openEditForm(username) {
        ...
        try {
            ...
            document.getElementById('fmUsername').disabled = true; // clé fonctionnelle non modifiable
            document.getElementById('fmProfile').value = u.profile;
            document.getElementById('fmComment').value = u.comment || '';
            if (u.expiry) {
                document.getElementById('fmExpiry').value = u.expiry.replace(' ', 'T').slice(0, 16);
            }
        } catch (e) {
    ...
    document.getElementById('userForm').addEventListener('submit', async (ev) => {
        ...
        const expiryRaw = document.getElementById('fmExpiry').value; // "YYYY-MM-DDTHH:MM"
