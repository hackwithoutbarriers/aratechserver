...
// Le token admin transite désormais uniquement en en-tête HTTP (X-Admin-Token),
// jamais dans l'URL — voir lib/api_client.php. Auparavant cette page utilisait
// file_get_contents($url . '&token=...') côté PHP ET répétait le token dans
// l'URL des appels fetch() côté JS, exactement le pattern que lib/api_client.php
// a été écrit pour remplacer (voir son docblock).
$apiHeadersJson = ara_api_headers_json($config);

$result = ara_api_call($config, 'admin');
$ads = $result['success'] ? ($result['data']['ads'] ?? []) : [];
            ...
            <div class="table-responsive">
                <table class="table table-striped mb-0">
                    <thead class="table-dark">
                        <tr>
                            <th>Titre</th><th>Type</th><th>Actif</th><th>Prix</th>
                            <th>Vues</th><th>Clics</th><th>Créé le</th><th style="width: 120px">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
...
document.getElementById('adForm').addEventListener('submit', function(e) {
    ...
    fetch(apiBase + '?route=admin_save_ad', {
        method: 'POST',
...
function deleteAd(id) {
    ...
    fetch(apiBase + '?route=admin_delete_ad', {
        method: 'POST',
        headers: Object.assign({ 'Content-Type': 'application/json' }, API_HEADERS),
...
function reseedAds() {
    ...
    fetch(apiBase + '?route=admin_reseed_ads', { method: 'POST', headers: API_HEADERS })
    .then(r => r.json())
    .then(data => {
        if (data.success) location.reload();
        else alert('Erreur : ' + (data.message || 'inconnue'));
