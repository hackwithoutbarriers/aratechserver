...
<?php else: ?>
    <div class="alert alert-info">
        ...
        (l'API confirme le contrat mais l'exécution routeur n'est pas encore branchée — un profil ne peut pas
        être créé/modifié à distance sans connexion Render→routeur, ce qui n'existe pas dans cette architecture).
        Cet onglet affiche l'état actuellement synchronisé depuis MikroTik.
    </div>

    <div class="card card-custom">
        <div class="card-header card-header-custom">Profils Hotspot (<?= count($profiles) ?>)</div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-striped mb-0">
                    <thead>
                        <tr>
                            <th>Nom</th><th>Utilisateurs partagés</th><th>Limite de débit</th>
                            <th>Pool d'adresses</th><th>Script on-login</th>
                        </tr>
                    </thead>
                    <tbody>
