...
$metricId = isset($metricId) ? (string)$metricId : '';
?>
<div class="card card-custom h-100 metric-card metric-card-<?= htmlspecialchars($metricTone, ENT_QUOTES, 'UTF-8') ?>">
    <div class="card-body d-flex align-items-start justify-content-between gap-3">
        <div class="min-w-0">
            ...
            <div class="stat-value mt-1"<?= $metricId !== '' ? ' id="' . htmlspecialchars($metricId, ENT_QUOTES, 'UTF-8') . '"' : '' ?>><?= htmlspecialchars($metricValue, ENT_QUOTES, 'UTF-8') ?></div>
            <?php if ($metricHelp !== ''): ?>
