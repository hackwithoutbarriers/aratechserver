...
$metrics = ['revenue'=>0,'tickets'=>0,'expenses'=>0,'profit'=>0,'margin'=>null,'average'=>0,'duplicates'=>0,'profile_stats'=>[],'sales'=>[],'daily'=>[],'error'=>null];
try {
    $pdo = ara_db_supabase();
...
<div class="business-command-center">
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        <div><div class="text-uppercase small fw-semibold text-muted mb-1">Business Intelligence</div><h2 class="mb-1">Business Command Center</h2><p class="text-muted mb-0">Indicateurs calculés à partir des ventes payées dédoublonnées.</p></div>
    ...
    <div class="row g-3">
        <?php $cards=[['Chiffre d’affaires',business_money($metrics['revenue']),'bi-cash-stack','success',$periodLabel],['Tickets vendus',number_format($metrics['tickets'],0,',',' '),'bi-ticket-perforated','info','Ventes payées uniques'],['Panier moyen',business_money($metrics['average']),'bi-calculator','primary','CA / tickets'],['Bénéfice',business_money($metrics['profit']),'bi-graph-up-arrow',$metrics['profit']>=0?'success':'danger','CA − dépenses']]; foreach($cards as [$metricLabel,$metricValue,$metricIcon,$metricTone,$metricHelp]): include __DIR__.'/../../components/metric-card.php'; endforeach; ?>
    </div>
    ...
    <div class="row g-3 mt-1">
        <div class="col-xl-8"><div class="card card-custom"><div class="card-header card-header-custom d-flex justify-content-between"><span><i class="bi bi-receipt"></i> Dernières ventes</span><a class="btn btn-sm btn-outline-light" href="business.php?tab=reports">Voir plus</a></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-striped mb-0"><thead><tr><th>Date</th><th>Utilisateur</th><th>Profil</th><th class="text-end">Montant</th></tr></thead><tbody><?php foreach($metrics['sales'] as $sale): ?><tr><td><?= htmlspecialchars((string)($sale['sale_date']??''),ENT_QUOTES,'UTF-8') ?> <?= htmlspecialchars((string)($sale['sale_time']??''),ENT_QUOTES,'UTF-8') ?></td><td><?= htmlspecialchars((string)($sale['username']??''),ENT_QUOTES,'UTF-8') ?></td><td><?= htmlspecialchars((string)($sale['profile']??''),ENT_QUOTES,'UTF-8') ?></td><td class="text-end"><?= business_money((int)($sale['amount']??0)) ?></td></tr><?php endforeach; ?><?php if(!$metrics['sales']): ?><tr><td colspan="4" class="text-center text-muted py-4">Aucune vente.</td></tr><?php endif; ?></tbody></table></div></div></div></div>
        <div class="col-xl-4"><div class="card card-custom h-100"><div class="card-header card-header-custom"><i class="bi bi-shield-check"></i> Qualité des données</div><div class="card-body"><div class="business-stat-row"><span>Lignes brutes<strong><?= number_format($metrics['tickets'] + $metrics['duplicates'],0,',',' ') ?></strong></span></div><div class="business-stat-row"><span>Ventes retenues<strong><?= number_format($metrics['tickets'],0,',',' ') ?></strong></span></div><div class="business-stat-row"><span>Doublons ignorés<strong><?= number_format($metrics['duplicates'],0,',',' ') ?></strong></span></div><div class="business-stat-row"><span>Dépenses<strong><?= business_money($metrics['expenses']) ?></strong></span></div><div class="business-stat-row"><span>Marge<strong><?= $metrics['margin']===null?'—':number_format($metrics['margin'],1,',',' ').' %' ?></strong></span></div></div></div></div>
    </div>
</div>
<script>
document.addEventListener('DOMContentLoaded', function(){const data=<?= json_encode($metrics['daily'],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) ?>; const el=document.getElementById('businessSalesChart'); if(el && typeof Chart!=='undefined'){new Chart(el,{type:'bar',data:{labels:data.map(x=>x.sale_date),datasets:[{label:'CA (FCFA)',data:data.map(x=>x.total)}]},options:{responsive:true,scales:{y:{beginAtZero:true}}}});}});
</script>
